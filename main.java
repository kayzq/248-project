...
public class main {
    ...
    static void writeNewCE(CRITICALLYENDANGEREDSIGHTING data) {
        ...
        static void writeNewTime(TimeSighting data) {
            try (PrintWriter writer = new PrintWriter(
                ...
                // After first write, set flag to false
    ...
    static void searchCE(LinkedList searchCE, String searchID) {
        String out = "";
        CRITICALLYENDANGEREDSIGHTING ce;
        boolean found = false;
        if (searchCE == null)
   ...
   public static String PrintTime(Queue queue) {
    ...
    if (queue.isEmpty()) {
        ...
        Queue tempQueue = new Queue(); // To preserve original data
        while (!queue.isEmpty()) {
            ts = (TimeSighting) queue.dequeue();
            out += ts.toString(); // Collect the string
            tempQueue.enqueue(ts); // Restore into temp
        }

        // Put everything back into original queue
    ...
    static double calcavg(Queue queue) {
        if (queue.isEmpty()) {
            return 0.0; // No sightings to calculate average
        }
        ...
        return  totalDuration / count ; // Return average duration
    }

   static double calctotal(Queue queue) {
    if (queue.isEmpty()) {
        return 0.0; // No sightings to calculate total
    }
